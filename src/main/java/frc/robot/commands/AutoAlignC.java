...
public class AutoAlignC extends Command{    
    ...
    @Override
    public void execute() {
    if (RobotContainer.rc_visionSS.isblue && RobotContainer.m_driverController.leftBumper().getAsBoolean()) {
    ...
    else if (!RobotContainer.rc_visionSS.isblue && RobotContainer.m_driverController.leftBumper().getAsBoolean()) {
        RobotContainer.m_robotDrive.drive(
            -MathUtil.applyDeadband(
                        RobotContainer.m_driverController.getLeftY(), OIConstants.kDriveDeadband),
                    -MathUtil.applyDeadband(
